...
def create_vtu_file(nNodes, value_mesh_twice_interp, filename, orig_vel, iTime):
...
nx = 221#55#111#221#55#221 #241 #I chose these values because it's easier for the CNN later.
ny = 42#42#206
nz = 1 # nz = 1 for 2D problems
ddx = np.array((0.01,0.01))
# the above should be set so that (nx-1)*ddx[0] = length of the domain in x direction = 2.2
...
for iGrid in range(nGrids):
    block_x_start = np.array(( grid_origin[0]+iGrid*grid_width[0], grid_origin[1] +iGrid*grid_width[1]))
...
for iTime in range(nTime):
    ...
    create_vtu_file(nNodes, reconstruction_on_mesh[iTime*nScalar:(iTime+1)*nScalar,:], template_vtu, original[:,iTime*nDim:(iTime+1)*nDim], iTime)
...
#    reconstruction =  np.dot( basis, np.dot( basis.T, snapshots_matrix ) ) 
#    reconstruction_data.append(reconstruction) 

#template_vtu = snapshot_data_location + snapshot_file_base + '0.vtu'
#for iTime in range(nTime):
#    for iField in range(nFields):
#        reconstruction = reconstruction_data[iField]
#        print ('recon shape',reconstruction.shape)
 #       reconstruction_grid = reconstruction.reshape(nScalar,nx,ny,nTime)
#        print ('recon shape',reconstruction.reshape(nScalar,nx,ny,nTime).shape)

#        zeros_on_grid = 1
#        reconstruction_mesh = u2r.interpolate_from_grid_to_mesh(reconstruction_grid[:,:,:,iTime], block_x_start, ddx, x_all, zeros_on_grid, nScalar,nx,ny,nz,nNodes,nDim, 1)
#        reconstruction_mesh = np.squeeze(reconstruction_mesh)
#        print('shape reconstruction mesh',reconstruction_mesh.shape)

#    create_vtu_file_timelevel(nNodes, reconstruction_mesh, template_vtu, iTime)   
