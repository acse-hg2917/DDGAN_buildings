...
def select_gridpoint():
    ...
    while np.sqrt(grid_origin[0]**2+grid_origin[1]**2) >= 1.3:
        ...
        grid_origin = [random.uniform(min_x+0.5, max_x-1.0), random.uniform(min_y+0.5, max_y-1.0)]
...
# nGrids = 1 # 4 or 1
# if nGrids==4:
#     nx = 55 #probably use square grid
#     ny = 42
...
# grid_origin = [0.0,0.0]
#find grid origin randomly between 0.3 and (2.7-0.5) to use inner grid
...
for iGrid in range (nGrids): #replace with ngrids later
    ...
    if iGrid%5 == 0:
        print("Currently Generating Grid ", iGrid)
...
print('basis_function.shape: ', basis_functions.shape)
...
nSGrids = 30
SSnapshots_matrix = np.zeros((nx*ny*3, nGrids*5))

for iGrid in nSGrids:
